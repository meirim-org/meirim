...
const Login = () => {
    ...
    const getIsEmailInvalid = React.useCallback(
        () => (onFocusInput.email ? false : !validateEmail(loginValues.email)),
        [loginValues.email, onFocusInput.email]
    );
    const formValidation = React.useCallback(() => {
        const isEmailInvalid = getIsEmailInvalid();
        const emailError = {
            isValid: !isEmailInvalid,
            message: !isEmailInvalid ? "" : "מייל לא תקין",
        };
    ...
    const validateBeforeFormSubmittion = (e) => {
        ...
        const emailError = {
            ...
            message: isEmailValid ? "" : "מייל לא תקין",
        };
        setFormErrors((ps) => ({ ...ps, emailError }));
    ...
    const submitLoginForm = async () => {
        ...
        const wrongCredintials =
            res && res.response && res.response.status === 403;
    ...
    return (
        <SC.MainWrapper>
            <SC.Titles>
                ...
                <SC.SubTitleWrapper>
                    <SC.SubTitle>
                        כדי להשלים את הפעולה עליכם להיות מחוברים
                    </SC.SubTitle>
                    <SC.SubTitle>
            ...
            <SC.ButtonWrapper>
                <Button
                    id="login-button"
                    text="התחברות למעירים"
                    onClick={validateBeforeFormSubmittion}
                />
            </SC.ButtonWrapper>
        </SC.MainWrapper>
    );
