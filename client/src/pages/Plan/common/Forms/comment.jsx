...
const CommentForm = ({ addNewComment, commentState, setCommentState }) => {
    ...
    const handleNewCommentType = (v) =>
        setCommentState((pv) => ({ ...pv, type: v }));
    const handleNewCommentText = (v) =>
        setCommentState((pv) => ({ ...pv, inputValue: v }));
    const newCommentViewHandler = () =>
        setCommentState((pv) => ({ ...pv, isOpen: !isOpen }));

    return (
        <SC.NewCommentTabPanel
            index={1}
            className={!comments ? "no-comments" : ""}
        >
            <>
