...
const formatNumber = (number, maximumSignificantDigits = 2) =>
    new Intl.NumberFormat('he-IL', { maximumSignificantDigits }).format(number);
...
const PlanCard = ({ plan }) => {
    ...
    const tagsRef = useRef([
        ...(plan?.tags || []).map(() => createRef()),
        createRef(),
    ]);
    const dispatch = useDispatch();
    const { isAuthenticated } = UserSelectors();
    const areaInDunam = plan?.data?.PL_AREA_DUNAM
        ? formatNumber(plan?.data?.PL_AREA_DUNAM)
        : 0;
    const housingUnitAddition =
        plan?.data?.QUANTITY_DELTA_120 > 0
            ? formatNumber(plan?.data?.QUANTITY_DELTA_120, 1)
            : 0;
    ...
    const intersectionObserverCallback = useCallback(
        (entries) => {
            ...
            if (visibleTags.length < entries.length) {
                newTags.splice(visibleTags.length - 1, 0, {
                    text: `${entries.length - visibleTags.length + 1}`,
                    type: 'plus',
                });
                setTags(newTags);
            }
    ...
    return (
        <Grid item xs={12} sm={6} md={4}>
            <SC.Card raised={true}>
                <Link className="card-link" to={`/plan/${plan.id}`}>
                    <SC.CardMedia title={plan.PL_NUMBER}>
                        <MapTitle>
                            <StatusChip>
                                <StatusDot
                                    approved={plan.status === 'מאושרות'}
                                />
                                <ChipText>
                                    {`${
                                        plan.status ?? ''
                        ...
                        <MapFooter>
                            <FooterChip>
                                <ChipText>{plan.PLAN_COUNTY_NAME}</ChipText>
                            </FooterChip>
                            {areaInDunam > 0 ? (
                                <FooterChip>
                                    <ChipText>{`${areaInDunam} דונם`}</ChipText>
                                </FooterChip>
                            ) : (
                                <div />
                            )}
                    ...
                    <SC.CardContent>
                        <PlanDetailsHeader>
                            ...
                            {housingUnitAddition > 0 && (
                                <span>{`${housingUnitAddition}+ דירות`}</span>
                            )}
                        </PlanDetailsHeader>
                        <PlanName>
                            {selectedLanguage === 'AR'
                                ? plan?.plan_display_name_arabic ||
                        ...
                        {plan?.goals_from_mavat && (
                            <PlanGoals>
                                {selectedLanguage === 'AR'
                                    ? removeTags(
                                          plan?.goals_from_mavat_arabic
                                      ) || removeTags(plan?.goals_from_mavat)
                                    : removeTags(plan?.goals_from_mavat)}
                            </PlanGoals>
                        )}
                        {tags.length > 0 && (
...
const BookmarkBtn = styled.button`
    ...
    background: center no-repeat
        url(${({ isBookmarked }) =>
            isBookmarked ? BookmarkFilledIcon : BookmarkOutlinedIcon})
        #ffffff;
    flex-shrink: 0;
