...
class LandUseVocabulary extends Component {
    ...
    renderUse(use) {
        ...
        if (!useTerms || !useTerms.length > 0) return <Chip label={use}></Chip>;
            ...
            <OverlayTrigger
                ...
                overlay={
                    <Tooltip id={`tooltip-${use}`}>
                        {useTerms.map((u) => {
                            return (
                                <span className="landUseTooltip">
                                    <strong className="landUseTooltip">
                                        {u.title}
                                    </strong>
                                    {u.description}
                                </span>
    ...
    finduse(useName) {
        return _.filter(terms, (t) => {
            return _.find(t.aliases, (a) => {
                return (
                    stringSimilarity.compareTwoStrings(a, useName) >
                    1 - 1 / a.length
                );
            });
        });
