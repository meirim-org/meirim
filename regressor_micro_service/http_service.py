...
class HttpService:
    ...
    def do_post(self, request):
        ...
        lst_of_plans = PlanRegressor.regression_for_all_plans(db)
        # Turn plans to data plans, this is done to expose only the relevant data to the clients
        lst_of_data_plans = [PlanData(pln) for pln in lst_of_plans]
...
class PlanData:
    """
    This class is used to store all the data about a single plan that should be returned in the HTTP response.
