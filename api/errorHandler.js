...
module.exports = {
  controllerErrorHandler: (req, res) => {
    ...
    if (err.code === 'ER_NO_REFERENCED_ROW' || err.code === 'ER_NO_REFERENCED_ROW_2') {
      // this is specific for MYSQL
      Log.info('BadRequest', 'Foreign key constraint could not be fulfilled', err.sqlMessage)
      return Failure.badRequest(res, 'Foreign key constraint could not be fulfilled')
    }
  ...
  generalErrorHandler: (error, req, res, next) => {
    ...
    if (error instanceof SyntaxError) {
      // this means body-parser could not parse request body
      Log.info('BadRequest', error.message, url)
      return Failure.badRequest(res, error.message)
    }
