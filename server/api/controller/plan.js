...
class PlanController extends Controller {
	browse(req) {
		...
		if (query.distancePoint) {
			...
			q.columns.push(
				Knex.raw(
					`ST_Distance(ST_GeomFromText(ST_AsText(geom),4326), ST_GeomFromText("${polygon}",4326))*${spatialUnitFactor} as distance`
				)
			);
	...
	async afterFetch(collection) {
		...
		try {
			const relevantTagsModels = await Tag.where("id", "IN", relevantTagIds).fetchAll({
				columns: ["id", "name"],
			});
	...
	publicBrowse(req) {
		...
		const polygon = wkt.convert(geojson);
		const whereRaw = [Knex.raw(`ST_Intersects(geom, ST_GeomFromText("${polygon}",4326))`)];
		const order = "-id";
