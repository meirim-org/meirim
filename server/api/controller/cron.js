      ...
      Bluebird.mapSeries(planCollection.models, plan => {
        ...
        return MavatAPI.getByPlan(plan)
          .then(mavatData => {
            ...
            return Promise.all([
              plan.save(),
              PlanAreaChangesController.refreshPlanAreaChanges(
                plan.id,
                plan.attributes.areaChanges,
              ),
            ]);
          })
          .catch(() => {
    ...
    .then(planCollection =>
      Bluebird.mapSeries(planCollection.models, plan => {
        ...
        return MavatAPI.getByPlan(plan).then(async mavatData => {
          ...
          await PlanAreaChangesController.refreshPlanAreaChanges(
            plan.id,
            plan.attributes.areaChanges,
          );
          Log.debug(
...
const alertToEmail = alert => {
  ...
  const alertTitle = `תוכניות חדשות בסביבת ${
    addressTitle || 'תחומי הענין שלך'
  }`;
  const mailSubject = `${alertTitle} | ${nowDate} `;
  return {
    alert: {
      title: alertTitle,
      unsubscribeLink: `${Config.get(
        'general.domain',
      )}alerts/unsubscribe/${alert.unsubsribeToken()}`,
    },
    mail: {
      subject: mailSubject,
    },
...
const sendDigestPlanningAlerts = async () => {
  ...
  try {
    ...
    const alertPlans = await Plan.getPlansByGeometryThatWereUpdatedSince(
      alertGeom,
      date,
    );
    ...
    try {
      if (alertPlans.length > 0)
        await DigestEmail.digestPlanAlert(
          email,
          emailPlanParams,
          emailAlertParams,
        );
      const newUpdateDate =
        alertPlans.length < maxAlertsToSend
          ? moment(max(map(alertPlans, 'created_at')))
          : moment(Date.now());
      alert.set({
    ...
    Log.debug(
      `User ${email} alert ${alert.id} with ${alertPlans[0].length} plans`,
    );
  } catch (e) {
...
const updatePlanTags = async () => {
  ...
  Log.info(
    `Done. Added tags to ${tagCounter}/${plans.models.length} plans. It took ${
      duration / 1000
    } seconds`,
  );
};
...
const fetchPlanStatus = () => {
  ...
  const lastVisitedDifference = Config.get(
    'planStatusChange.lastVisitedDifference',
  ); //days
  ...
  return Plan.query(qb => {
    qb.leftJoin(
      ...
      'plan.status',
      '=',
      'status_mapping.mavat_status',
    )
      .whereRaw(
        `(status_mapping.meirim_status is null or status_mapping.meirim_status != '${meirimStatuses.APPROVED}') and plan.MP_ID NOT LIKE \'NOT_FOUND\' and (plan.last_visited_status < '${dateString}' OR plan.last_visited_status IS NULL)`,
      )
    ...
    .then(planCollection =>
      Bluebird.mapSeries(planCollection.models, plan => {
        ...
        return MavatAPI.getPlanStatus(plan)
          .then(async planStatuses => {
            try {
              // cut description to match db field length
              planStatuses.forEach(ps => {
                if (ps.attributes.status_description) {
                  ps.attributes.status_description = ps.attributes.status_description.substr(
                    0,
                    254,
                  );
                }
              });
              const mostRecent = planStatuses.sort((statusA, statusB) => {
                Date.parse(statusB.attributes.date) -
                  Date.parse(statusA.attributes.date);
              });
              ...
              if (mavatStatus === null) {
                const res = await PlanStatusChange.byMeirimStatus(
                  meirimStatuses.PUBLIC_OBJECTION,
                );
...
function shouldSendEmail(plan, planStatuses, mavatStatus) {
  ...
  if (!plan.attributes.was_deposited) {
    for (var i = 0; i < planStatuses.length; ++i) {
      ...
      if (
        Boolean(status.attributes.status) &&
        mavatStatus.indexOf(status.attributes.status) >= 0
      ) {
        sendEmail = true;
      }
    }
...
const fillMPIDForMissingPlans = async () => {
  try {
    const columnsToFetch = ['PL_NUMBER', 'MP_ID'];
    const limit = 20;
    ...
    if (!models || size(models) === 0) return;
    // Now querying the BlueLines API to see if there are MP_ID
    const planNumbers = models.map(p =>
      encodeURIComponent(p.attributes.PL_NUMBER),
    );
    const seperateWithDelimiter = `'${planNumbers.join("', '")}'`;
    const blueLinesQuery = iplanApi.buildMavatURL(
      1,
      columnsToFetch,
      `PL_NUMBER in (${seperateWithDelimiter})`,
      'true',
    );
    const { data } = await axios.get(blueLinesQuery);
    const plNumberToMpIDMap = reverse(data.features).reduce(
      (acc, curr) => ({
        ...acc,
        [curr.attributes.PL_NUMBER]: curr.attributes.MP_ID,
      }),
      {},
    );
    console.log(
      `Filling MP_ID found ${size(plNumberToMpIDMap)} ids out of ${limit}`,
    );
