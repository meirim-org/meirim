...
class Email {
  ...
  treeAlert(user, unsentTree, treeStaticMap) {
    ...
    data.place_text = data.place
      ? `רשיון כריתה חדש ב${data.place}`
      : 'רשיון כריתה חדש באזורך';
    data.address = data.street
      ? data.street_number
        ? `${data.street} ${data.street_number}`
        : `${data.street}`
      : 'לא צוינה כתובת';
    data.total_trees_text =
      data.total_trees === 1 ? 'עץ אחד' : `${data.total_trees} עצים`;
    data.reason_short_text = data.reason_short
      ? data.reason_short
      : 'לא צוינה סיבה';
    data.reason_detailed_text = data.reason_detailed
      ? data.reason_detailed
      : 'לא צוין פירוט הסיבה';
